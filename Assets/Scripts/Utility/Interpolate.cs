 ...
 * Bezier(), and CatmullRom(). High level access using value generators,
 * NewEase(), NewBezier(), and NewCatmullRom().
 *
 * Sequence generators are typically used as follows:
 *
 * IEnumerable<Vector3> value = Interpolate.New[Ease|Bezier|CatmulRom](configuration);
 * foreach (Vector3 newPoint in value) {
 ...
 * Or:
 *
 * IEnumerator<Vector3> value = Interpolate.New[Ease|Bezier|CatmulRom](configuration).GetEnumerator();
 * function Update() {
 *   if (value.MoveNext()) {
 *     transform.position = value.Current;
 *   }
 * }
 *
 ...
 */
    ...
    */
	...
	/**
     * Generates value of integers from start to end (inclusive) one step
     * at a time.
     */
	...
	/**
     * Returns value generator from start to end over duration using the
     * given easing function. The value is generated as it is accessed
	...
	/**
     * Generic easing value generator used to implement the time and
     ...
     */
	static Vector3 Ease(Function ease, Vector3 start, Vector3 end, float elapsedTime, float duration) {
		start.x = ease(start.x, end.x, elapsedTime, duration);
		start.y = ease(start.y, end.y, elapsedTime, duration);
		start.z = ease(start.z, end.z, elapsedTime, duration);
		return start;
	}
	...
	/**
     * Returns value generator from the first node to the last node over
     ...
     * in the value. If there are no control points (ie. only two nodes, first
     * and last) then this behaves exactly the same as NewEase(). In other words
     * a zero-degree bezier spline curve is just the easing method. The value
	...
	/**
     * Generic bezier spline value generator used to implement the time and
     * slice variants. Normally you would not use this function directly.
     */
	...
	/**
     * Returns value generator from the first node, through each control point,
	...
	/**
     * Generic catmull-rom spline value generator used to implement the
